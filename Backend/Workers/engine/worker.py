...
RABBITMQ_URL     = __import__('os').environ.get("RABBITMQ_URL", "amqp://guest:guest@localhost:5672/")
COOLDOWN_SECONDS = 10
FRAME_SKIP       = 3
...
class PipelineWorker:
    def __init__(self, pipeline_id: int, camera_id: int, ml_model_id: int,
        ...
        self.violation_rules = violation_rules        # ✅ injected per usecase
    ...
    def _process(self, ch, method, body, model):
        ...
        for rule in self.violation_rules:
            if rule["required_labels"].issubset(detected_labels):
                send_violation(
                    self.pipeline_id, self.camera_id, self.ml_model_id,
                    rule["violation_type"], annotated_b64, detections,
                )
